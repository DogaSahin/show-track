...
class TokenRefreshAuthenticator
    ...
    constructor(
        ...
        // ReturnCount: three exits, and each one is a distinct "do not retry" decision — the
        // retry bound, the refresh failure, and the success path. TooGenericExceptionCaught /
        ...
        override fun authenticate(
            ...
            val fresh =
                runBlocking {
                    mutex.withLock {
                        // SINGLE-FLIGHT. Five parallel requests hit one expired token and all 401
                        // at once. Without this check each would refresh; the backend stores only
                        // the refresh token's HASH and rotates on use, so the losers invalidate
                        // the winner's token and log the user out mid-session. Re-read inside the
                        // lock: if another caller already refreshed, the stored token differs from
                        // the one we sent and we just use it.
